    ...
    auth.run(credentials, account, session, function(err) {
      if(err) {
        ...
        if(ConfMgr.readConf('notifications.notifOnLogin')) {
          ...
          logger.notify(email, 'Connexion notification',
            'Hi,\n\n' +
    ...
    if(err) {
      ...
      auth.run(credentials, account, session, function(err) {
        if(err) {
          ...
          account.maxSecrets = ConfMgr.readConf('application.defaultMaxSecrets') || 5

          logger.notice('new elipsis user', email)

          PaymentGateway.registerUser(account, function(err) {
