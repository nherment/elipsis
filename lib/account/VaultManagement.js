...
VaultManagement.removeVault = function(ipAddress, email, vaultUid, callback) {
  ...
  DBHelper.Account.findByKey(email, function(err, account) {
    if(err) {
      ...
      var vaultIndex = account.vaults.indexOf(vaultUid)

      if(vaultIndex > -1) {

        logger.audit(email, AuditActions.VAULT_DELETE, {date: new Date(), ip: ipAddress})
        ...
        DBHelper.Vault.remove({uid: vaultUid}, function(err) {
          ...
          if(err) {
            ...
            account.vaults = account.vaults.splice(vaultIndex, 1)

            DBHelper.Account.save(account, function(err, updatedAccount) {
              logger.audit(email, AuditActions.VAULT_DELETE, {date: new Date(), ip: ipAddress, vaultUid: vaultUid})
              callback(undefined)
            })
