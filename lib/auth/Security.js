...
if(hashIterations < 20000) {
  logger.warn('Password hashing uses too few iterations. A high number of iterations ensure it is hard to brutefore a password. A number of at least ['+20000+'] is recommended')
}
...
function verify(plainText, hashDetails, callback) {

  if(hashDetails && hashDetails.salt && hashDetails.iterations && hashDetails.keylen && hashDetails.hash) {

    hash(plainText, new Buffer(hashDetails.salt, 'hex'), hashDetails.iterations, hashDetails.keylen, function(err, hash) {
      if(err) {
        logger.error(err)
        callback(false)
      } else {
        callback(hash.toString('hex') === hashDetails.hash)
      }
    })
